...
async function findRegistrationByEmail(email) {
  const baseUrl = process.env.CELLXPERT_BASE_URL || 'https://affiliate.marketsvox.com/api/';
  ...
  if (response.status < 200 || response.status >= 300) {
    throw new Error(`Cellxpert request failed: ${response.status} ${response.statusText}`);
  }
  ...
  if (trimmed === 'Bad Authentication Key') {
    throw new Error('Cellxpert authentication failed. Check CELLXPERT_API_KEY and Fixie IP whitelisting.');
  }
  ...
  const matchingRows = getMatchingRegistrationRows(rows, email);
  return { found: matchingRows.length > 0, rows, matchingRows };
}
